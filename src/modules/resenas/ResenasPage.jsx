    ...
    <div className="productos-module">
      ...
      <div className="card" style={{marginTop:12}}>
        <div className="productos-filters" style={{display:'flex', gap:12, alignItems:'center', flexWrap:'wrap'}}>
          ...
          <button className="btn" onClick={async ()=>{ setAnalisisError(''); setAnalisisLoading(true); try { const data = await analisisIAService.resumenOpinionProducto(Number(analisisProductId)); setAnalisisResumen(data); toast.success('Resumen cargado'); } catch(e){ setAnalisisError(e.message||'Error'); toast.error('Error al cargar resumen'); } finally { setAnalisisLoading(false) } }} style={{background:'#1f2937', color:'#fff'}}>Ver resumen de opiniones</button>
          <button className="btn" onClick={async ()=>{ setAnalisisError(''); setMalasLoading(true); try { const data = await analisisIAService.sincronizarOpinionesProductos(); setMalasCalificaciones(data); toast.success('Opiniones sincronizadas'); } catch(e){ setAnalisisError(e.message||'Error'); toast.error('Error al sincronizar'); } finally { setMalasLoading(false) } }} style={{background:'#ef4444', color:'#fff'}}>Sincronizar opiniones</button>
        </div>
