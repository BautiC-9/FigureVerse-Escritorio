...
const SoporteController = {
  ...
  async estadisticas(req, res) {
    try { const r = await soporteService.estadisticas(req); return res.json(r) }
      ...
      try {
        const l = await soporteService.listarTodos(req, { page: 1, limit: 500 })
        const arr = Array.isArray(l?.data) ? l.data : (Array.isArray(l) ? l : [])
        const total = arr.length
        const abiertos = arr.filter(t => String(t.estado||'').toLowerCase()!=='cerrado').length
        const cerrados = arr.filter(t => String(t.estado||'').toLowerCase()==='cerrado').length
        return res.json({ total, abiertos, cerrados })
      } catch (err) { return res.status(400).json({ error: e.message }) }
    }
  },
  // Responder
